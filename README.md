...
Debug Z80 programs running on retro computers directly from Neovim: breakpoints, source-level stepping, registers and live memory inspection through [nvim-dap](https://github.com/mfussenegger/nvim-dap). Currently supports [ZEsarUX](https://github.com/chernandezba/zesarux) and [MAME](https://www.mamedev.org/) as debugging backends, with source-level debugging for [sjasmplus](https://github.com/z00m128/sjasmplus) and [SDCC](http://sdcc.sourceforge.net/) projects. Amstrad CPC and ZX Spectrum are included as working examples.
...
```lua
{
  ...
  config = function()
    ...
    retro.setup({
      -- Override the file extension → adapter mapping (optional)
      ext_map = {
        z80 = "zesarux",
        s80 = "zesarux",
        asm = "zesarux",
      },
      -- Memory dump panel options (optional)
      memory = {
...
`src/` and `build/` are just a convention — you can use any structure, as long as the paths in `launch.json` match.
  ...
  "configurations": [
    {
      ...
      "loadAddress": "0x4000",
      "zesaruxArgs": ["--machine", "cpc6128", "--snap-no-change-machine", "--disable-autoframeskip"],
      "preLaunchTask": "build"
    }
  ]
}
...
The adapter connects to `localhost:10000` in this manually-managed case. Note this is a single shared ZEsarUX process — unlike the auto-launched case above, a second concurrent session attaching here can still corrupt the first (see the "Manually-managed ZEsarUX" item in [TODO.md](TODO.md)).
...
| `mamePort` | gdbstub port. Defaults to `2159`. |
